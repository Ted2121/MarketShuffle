...
export default function TwReports() {
    ...
    const addReport = () => {
        if (reportInput.trim()) {
            const { content, date, links } = parseReport(reportInput.trim());
            const newReport = { content, date, links };
    ...
    const parseReport = (reportString) => {
        ...
        // If a date was found, create a content string by removing the date
        const content = date ? reportString.replace(dateMatch[0], '').trim() : reportString.trim();
